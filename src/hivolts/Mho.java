...
public class Mho extends MovePiece {
    ...
    private void inLine() {
        //Checks if hivolts.Mho is in line with player. Must move in that direction
    ...
    private void diagonalMove() {
        // Checks if hivolts.Mho can move diagonally onto open space
    ...
    private void horizontalMove() {
        // Checks if hivolts.Mho can move horizontally or vertically onto open space
    ...
    private void diagonalOntoFence() {
        //Checks if hivolts.Mho can move onto diagonally fence
        if (player.x > x) {
            if (player.y > y) {
                if (checkFence(x + 1, y + 1) && !checkMho(x + 1, y + 1)) { //Checks for fence and not hivolts.Mho
                    killMho();
                } else {
                    horizontalOntoFence();
                }
    ...
    private void horizontalOntoFence() {
        // Checks if hivolts.Mho can move either horizontally or vertically onto a fence
        if (Math.abs(player.x - x) >= Math.abs(player.y - y)) { // if vert dist greater than horiz dist
            if (player.x > x) { // if mho is under player
                if (checkFence(x + 1, y) && !checkMho(x + 1, y)) { //Checks if there is a fence and no hivolts.Mho
                    killMho();
                }
            } else if (player.x < x) {
                if (checkFence(x - 1, y) && !checkMho(x - 1, y)) {
    ...
    private boolean checkMho(int xloc, int yloc) {
        //Return true if an hivolts.Mho is at xloc,yloc
        boolean retval = false;
